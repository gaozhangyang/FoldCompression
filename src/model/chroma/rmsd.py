...
from . import graph
from .linalg import eig_leading
from . import geometry
from . import protein_graph

def batched_eigh(F: torch.Tensor, chunk_size: int = 10000):
    """
    对形状 [B, n, n] 的大批量 F 进行分块 eigendecomposition，
    返回 L: [B, n], V: [B, n, n]
    """
    Ls, Vs = [], []
...
class CrossRMSD(nn.Module):
    ...
    def eigen(self, F):
        ...
        if self.method == "symeig":
            ...
            L, V = batched_eigh(F, chunk_size=10000)
            top_eig = L[:, 3]
            vec = V[:, :, 3]
        elif self.method == "power":
            top_eig, vec = eig_leading(F, num_iterations=self.method_iter)
    ...
    def pairedRMSD(
        ...
        R_to_F = self.R_to_F.type(R_flat.dtype).to(R_flat.device)
...
class BackboneRMSD(nn.Module):
    ...
    def align(self, X_mobile, X_target, C, align_unmasked=False):
        ...
        mask_flat = mask.unsqueeze(-1).expand(-1, -1, X_mobile.shape[-2]).reshape(mask.shape[0], -1)
