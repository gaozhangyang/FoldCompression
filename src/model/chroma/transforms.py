...
def transform_flatten(C, data):
    assert C.dim() in [2, 3], "C must be a 2D or 3D tensor."
    B = C.size(0)
    
    # 计算每个批次的最大索引
    if C.dim() == 2:
        max_per_batch = torch.max(C, dim=1).values  # [B]
    else:  # C.dim() == 3
        max_per_batch = torch.amax(C, dim=(1, 2))  # [B]
    
    # 计算偏移量
    shift = torch.cat([
        torch.zeros(1, device=C.device),
        (max_per_batch + 1).cumsum(dim=0)[:-1]
    ])
    ...
    # 应用偏移并展平C
    C_shifted = C + shift
    C_flatten = C_shifted.view(-1)
    data_flatten = data.view(-1, data.size(-2), data.size(-1))
    
    # 展平数据
    if data is not None:
        
        return C_flatten, data_flatten
    else:
        return C_flatten


def group_and_pad(C, D):
    # 获取唯一的batch编号和映射到连续的batch_id
    unique_ids, inverse_indices = torch.unique(C, sorted=True, return_inverse=True)
    B = len(unique_ids)
    ...
    for batch_id in range(B):
        mask = (inverse_indices == batch_id)
        groups_C.append(C[mask])
        groups_D.append(D[mask])
    
    # 计算最大长度L
    lengths = [len(g) for g in groups_C]
    L_max = max(lengths) if lengths else 0
    
    # 初始化填充后的矩阵和掩码
    padded_C = torch.zeros((B, L_max), dtype=C.dtype, device=C.device) + torch.nan
    padded_D = torch.zeros((B, L_max, D.size(-1)), dtype=D.dtype, device=D.device) + torch.nan
    mask = torch.ones((B, L_max), dtype=torch.int, device=C.device)
    
    # 填充数据并设置掩码
    for i, (c_group, d_group) in enumerate(zip(groups_C, groups_D)):
        L = len(c_group)
        padded_C[i, :L] = c_group
        padded_D[i, :L] = d_group
        mask[i, L:] = 0  # 填充位置设为0
    
    return padded_C, padded_D, mask

def transform_cbach_to_sbatch(C, data):
    C_flatten, data_flatten = transform_flatten(C, data)
    C_flatten = C_flatten + 1
    N = C_flatten.shape[0]
    dim = data.shape[-1]
    data_flatten = data_flatten.reshape(N,-1)
    C_batch, data_batch, mask_batch = group_and_pad(C_flatten, data_flatten)
    mask_batch = mask_batch.bool() & ~torch.isnan(data_batch.sum(dim=-1))
    data_batch = data_batch.reshape(C_batch.shape[0], -1, dim)
    mask_batch = mask_batch.unsqueeze(-1).repeat(1,1,data.shape[-2])
    mask_batch = mask_batch.reshape(C_batch.shape[0], -1)
    # data_batch[torch.isnan(data_batch)] = 0
    
    return C_batch, data_batch, mask_batch