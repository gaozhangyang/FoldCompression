...
def apply_rotary_emb_torch(x, cos, sin, interleaved=False, _inplace=False):
    ...
    cos = repeat(cos, "b s d -> b s 1 (2 d)")
    sin = repeat(sin, "b s d -> b s 1 (2 d)")
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def _update_cos_sin_cache(self, position, seqlen, device=None, dtype=None):
        ...
        if (
            ...
            if self.pos_idx_in_fp32:
                ...
                # t = position.to(self.inv_freq.dtype)
                t /= self.scaling_factor
                inv_freq = self.inv_freq
    ...
    def forward(
        self, position, q: torch.Tensor, k: torch.Tensor, seqlen_offset: int = 0
        ...
        # )
        # self._update_cos_sin_cache(
        #     position, q.shape[1] + seqlen_offset, device=q.device, dtype=q.dtype
        # )
        seq_len = 4096
        self._update_cos_sin_cache(
            position, seq_len, device=q.device, dtype=q.dtype
        )
        ...
        if self.scale is None:
            return (
                apply_rotary_emb_torch(
                    ...
                    self._cos_cached[position],
                    self._sin_cached[position],
                    self.interleaved,
                    True,  # inplace=True
                ),
                apply_rotary_emb_torch(
