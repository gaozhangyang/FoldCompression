...
def randomize_per_residue(atom_data):
    # 假设 atom_data 是一个形状为 (L, 4, 3) 的 numpy 数组（L 个氨基酸，每个 4 个原子，3D 坐标）
    mask = torch.zeros_like(atom_data)

    for i in range(atom_data.shape[0]):  # 遍历每个氨基酸
        keep = [False, True, False, False]  # 强制包含 CA（索引 1）
        for j in [0, 2]:  # N, C, O 随机决定
            keep[j] = random.random() < 0.8  # 80% 概率包含
        for j in [3]:  # N, C, O 随机决定
            keep[j] = random.random() < 0.3  # 30% 概率包含
        for j in range(4):
            if keep[j]:
                mask[i, j, :] = 1
    return mask

def randomize_global_mask(atom_data):
    keep = [False, True, False, False]  # 保证 CA（索引 1）

    for j in [0, 2]:
        keep[j] = random.random() < 0.8  # 50% 概率包含
